...
def convert_pdf_to_words(file_name:str)-> list:
    ...
    all_words_list = []
    with pymupdf.open(f"{FOLDER_IN}{file_name}") as doc: # open a document
         for page in doc: # iterate the document pages
            full_words = page.get_text("words")
            # PARSE WORDS:
...
def parse_jad_word_list(word_list:list)-> dict:
    """Extracts data from a JAD word_list
        Returns: dictionary"""
    jad_dict = {}
    # PARSE LIST HERE
    return jad_dict
